-- 0002_qa_replies.sql — adds reply support and Q&A moderation to session_questions.
-- Apply with: psql "$SUPABASE_DB_URL" -f supabase/migrations/0002_qa_replies.sql
...
-- Moderation fields on existing session_questions
...
-- Auto-flag replies as official when poster is a session speaker or organizer/admin
...
create policy "replies_select_all" on public.question_replies for select to authenticated using (true);

drop policy if exists "replies_insert_self" on public.question_replies;
create policy "replies_insert_self" on public.question_replies for insert to authenticated
  with check (user_id = auth.uid());

drop policy if exists "replies_update_self_or_organizer" on public.question_replies;
create policy "replies_update_self_or_organizer" on public.question_replies for update to authenticated
...
create policy "replies_delete_self_or_organizer" on public.question_replies for delete to authenticated
  using (user_id = auth.uid() or public.is_organizer());

drop policy if exists "reply_upvotes_select_all" on public.reply_upvotes;
create policy "reply_upvotes_select_all" on public.reply_upvotes for select to authenticated using (true);

drop policy if exists "reply_upvotes_own" on public.reply_upvotes;
create policy "reply_upvotes_own" on public.reply_upvotes for all to authenticated
...
begin
  if not exists (
    ...
    where pubname = 'supabase_realtime' and tablename = 'question_replies'
  ) then
    alter publication supabase_realtime add table public.question_replies;
  end if;
  if not exists (
    select 1 from pg_publication_tables
    where pubname = 'supabase_realtime' and tablename = 'reply_upvotes'
  ) then
